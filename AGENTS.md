...
同时内置书签管理功能，支持 AI 自动分类。使用 WXT + Vue 3 + TypeScript 构建。
...
- `entrypoints/options/App.vue` — 设置页（subDir、imageMode、OSS 配置、AI 配置、书签配置、Get笔记配置）
- `entrypoints/bookmarks/App.vue` — 书签管理页，含文件夹树、书签列表、AI 分类侧边栏；中间书签栏支持 `原始 / 域名` 排序切换
...
- `settings.ts` — 用 `browser.storage.local` 持久化设置（`Settings` 接口，含书签、AI 配置和 Get笔记配置）
...
- `aliyun.ts` — `OpenAICompatibleProvider` 实现（兼容 OpenAI Chat API 格式，默认配置指向阿里云通义千问，也支持其他 OpenAI 兼容服务；聊天补全请求显式传 `enable_thinking: false`）
- `index.ts` — 工厂函数 `createAIProvider(settings)`
